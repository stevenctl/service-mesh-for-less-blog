...
{{ toYaml $extAuth.deployment.resources | indent 4}}
  ...
  {{- /* because gloo.containerSecurityContext can return an empty line, it can add trailing whitespace when the parent gloo.extauthcontainer is indented so we catch it here to avoid that */ -}}
  {{- $spec := include "gloo.containerSecurityContext" (dict "values" $extAuth.deployment.extAuthContainerSecurityContext "defaults" (dict) "podSecurityStandards" .Values.global.podSecurityStandards "globalSec" .Values.global.securitySettings) | trim }}
  {{- with $spec -}}
  {{- . | nindent 2 -}}
  {{- end }}
  env:
...
{{/* Helper used to properly set the ProxiesToCreateDataplaneFor value at the top scope.
     ...
     If kubeGateway is enabled we need to render standard dataplane components regardless of whether
     dataplanePerProxy is enabled, so we add a placeholder proxy to render in that case.
     The placeholder is `kube-gateway-placeholder` and it is a reserved value. If there is a user-specified gatewayProxy
     with the name kubeGatewayPlaceholder and dataplanePerProxy is enabled there will be unexpected behavior.
     Every template that names resources based on whether dataplanePerProxy is set also checks for this placeholder.
     We also ensure that we render at most one set of standard dataplane components by only selecting
     an enabled "classic" Gateway Proxy to use to render if we do not already have one from kubeGateway
     being enabled.
...
{{ $proxiesToCreateDataplaneFor = set $proxiesToCreateDataplaneFor "kube-gateway-placeholder" dict }}
...
{{- end }}{{/* if $.Values.global.extensions.dataplanePerProxy */}}
{{- if not (len $proxiesToCreateDataplaneFor) }}
{{/*
...
{{- end }}{{/* if not $gwProxy.disabled */}}
{{- end }}{{/* range */}}
{{- if len (keys $nonDisabledGwProxies) }}
{{- $firstKey := keys $nonDisabledGwProxies | sortAlpha | first }}
{{- $proxiesToCreateDataplaneFor = pick $nonDisabledGwProxies $firstKey }}
{{- end }}{{/* if len (keys $nonDisabledGwProxies) */}}
{{- end }}{{/* if not(len $proxiesToCreateDataplaneFor) */}}
{{- $_ := set $ "ProxiesToCreateDataplaneFor" $proxiesToCreateDataplaneFor -}}
{{- end }} {{/* define "gloo.dataplaneperproxyhelper */}}
